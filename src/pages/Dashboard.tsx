...
import ReferralCenter from './dashboard/ReferralCenter';
    ...
    <div className="flex min-h-screen bg-[#060608] text-white font-sans">
      ...
      <main className="flex-1 md:ml-64 min-h-screen relative overflow-x-hidden">
        ...
        <div className="md:hidden glass-strong sticky top-0 z-30 px-4 py-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <div className="glass rounded-lg px-3 py-1.5 flex items-center gap-1.5 relative group cursor-pointer" title={
              walletSummaries 
                ? walletSummaries.map(s => `${s.asset_code}: ${s.total_balance}`).join('\n') 
                : 'Total usable balance'
            }>
        ...
        {isMobileMenuOpen && (
          <div className="md:hidden fixed inset-0 z-50 flex">
            ...
            ></div>
            <div className="relative w-64 max-w-[80%] h-full bg-[#060608] border-r border-white/10 flex flex-col p-5 shadow-2xl transform transition-transform duration-300 ease-in-out">
              ...
              <div className="glass rounded-xl p-4 mb-6 relative border border-white/5">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-sm font-bold text-gray-400">{tDash('dash.credits')}</span>
                  <span className="px-2 py-0.5 rounded-full bg-primary-500/20 text-primary-400 text-[10px] font-bold uppercase tracking-wider">{plan}</span>
                </div>
                <h3 className="text-xl font-bold mb-2">
                  <span className="text-primary-400">{credits !== null ? credits : '...'}</span>
                  <span className="text-sm font-normal text-gray-500 ml-1">/{maxCredits !== null ? maxCredits : '...'} cr</span>
                </h3>
                <div className="w-full bg-black/40 rounded-full h-1.5 mb-1 overflow-hidden">
                  <div className="bg-gradient-to-r from-primary-500 to-yellow-400 h-full" style={{ width: `${creditPercent}%` }}></div>
                </div>
              </div>

              <nav className="space-y-1 flex-1">
                <button onClick={() => { setActiveSection('home'); setIsMobileMenuOpen(false); }} className={`w-full text-left px-3 py-3 rounded-lg text-sm transition-all flex items-center gap-3 ${activeSection === 'home' ? 'text-white bg-primary-500/15 border border-primary-500/30' : 'text-white/50 hover:text-white hover:bg-white/5'}`}>
                  <svg width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><path d="M3 9l9-7 9 7v11a2 2 0 01-2 2H5a2 2 0 01-2-2z"/><polyline points="9,22 9,12 15,12 15,22"/></svg>
                  <span>{tDash('dash.nav.home')}</span>
                </button>
                <button onClick={() => { setActiveSection('create'); setIsMobileMenuOpen(false); }} className={`w-full text-left px-3 py-3 rounded-lg text-sm transition-all flex items-center gap-3 ${activeSection === 'create' ? 'text-white bg-primary-500/15 border border-primary-500/30' : 'text-white/50 hover:text-white hover:bg-white/5'}`}>
                  <svg width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="16"/><line x1="8" y1="12" x2="16" y2="12"/></svg>
                  <span>{tDash('dash.nav.create')}</span>
                </button>
                <button onClick={() => { setActiveSection('history'); setIsMobileMenuOpen(false); }} className={`w-full text-left px-3 py-3 rounded-lg text-sm transition-all flex items-center gap-3 ${activeSection === 'history' ? 'text-white bg-primary-500/15 border border-primary-500/30' : 'text-white/50 hover:text-white hover:bg-white/5'}`}>
                  <svg width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><polyline points="22,12 18,12 15,21 9,3 6,12 2,12"/></svg>
                  <span>{tDash('dash.nav.history')}</span>
                </button>
                {hasReferralAccess && (
                  <button onClick={() => { setActiveSection('referral'); setIsMobileMenuOpen(false); }} className={`w-full text-left px-3 py-3 rounded-lg text-sm transition-all flex items-center gap-3 ${activeSection === 'referral' ? 'text-white bg-primary-500/15 border border-primary-500/30' : 'text-white/50 hover:text-white hover:bg-white/5'}`}>
                    <Users width="16" height="16" className="text-current" />
                    <span>{t('ref.title')}</span>
                  </button>
                )}
                <button onClick={() => { setActiveSection('settings'); setIsMobileMenuOpen(false); }} className={`w-full text-left px-3 py-3 rounded-lg text-sm transition-all flex items-center gap-3 ${activeSection === 'settings' ? 'text-white bg-primary-500/15 border border-primary-500/30' : 'text-white/50 hover:text-white hover:bg-white/5'}`}>
                  <svg width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="3"/><path d="M19.4 15a1.65 1.65 0 00.33 1.82l.06.06a2 2 0 010 2.83 2 2 0 01-2.83 0l-.06-.06a1.65 1.65 0 00-1.82-.33 1.65 1.65 0 00-1 1.51V21a2 2 0 01-4 0v-.09A1.65 1.65 0 009 19.4a1.65 1.65 0 00-1.82.33l-.06.06a2 2 0 01-2.83 0 2 2 0 010-2.83l.06-.06A1.65 1.65 0 004.68 15a1.65 1.65 0 00-1.51-1H3a2 2 0 010-4h.09A1.65 1.65 0 004.6 9a1.65 1.65 0 00-.33-1.82l-.06-.06a2 2 0 010-2.83 2 2 0 012.83 0l.06.06A1.65 1.65 0 009 4.68a1.65 1.65 0 001-1.51V3a2 2 0 014 0v.09a1.65 1.65 0 001 1.51 1.65 1.65 0 001.82-.33l.06-.06a2 2 0 012.83 0 2 2 0 010 2.83l-.06.06A1.65 1.65 0 0019.4 9a1.65 1.65 0 001.51 1H21a2 2 0 010 4h-.09a1.65 1.65 0 00-1.51 1z"/></svg>
                  <span>{tDash('dash.nav.settings')}</span>
                </button>
              </nav>

              <button onClick={() => { setIsMobileMenuOpen(false); handleLogout(); }} className="mt-auto flex items-center gap-2 text-sm text-gray-500 hover:text-red-400 transition py-3 border-t border-white/10">
        ...
        <div className="p-6 sm:p-8 max-w-5xl relative z-10">
          ...
          {activeSection === 'referral' && hasReferralAccess && (
            <ReferralCenter canManageReferral={canManageReferral} />
          )}
