...
import type { ReferralOverview, ReferralCode, ReferralConversion, Commission, ReferralCodeResolve } from '@/types/referral';
...
export default function ReferralCenter({ canManageReferral }: ReferralCenterProps) {
  ...
  useEffect(() => {
    if (activeCode) {
      referralService.resolveCode(activeCode)
        .then(res => {
          if (res?.data) setResolvedCode(res.data);
        })
        .catch(err => console.error('Failed to resolve code:', err));
    } else {
  ...
  const handleRedeem = async () => {
    ...
    try {
      ...
      console.error(err);
      const errMsg = err instanceof Error ? err.message : 'Failed to redeem';
      // Specific handling for NO_REDEEMABLE_COMMISSION can be done globally in api.ts, or mapped here.
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 max-w-5xl mx-auto space-y-8 pb-10">
      ...
      <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-primary-600 via-purple-600 to-blue-700 p-8 sm:p-10 shadow-2xl shadow-primary-500/20">
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-8">
          ...
          <div className="w-full md:w-auto flex-shrink-0 bg-white/10 backdrop-blur-md p-5 rounded-2xl border border-white/20 shadow-xl">
            ...
            {resolvedCode && resolvedCode.reward_policy_desc && (
              ...
              <div className="flex items-center justify-between">
                ...
                {overview && overview.redeemable_commission > 0 && (
                  <button
                    ...
                    className="flex items-center gap-1.5 px-3 py-1.5 bg-primary-500 hover:bg-primary-600 text-white rounded-lg text-xs font-bold transition-colors shadow-[0_0_15px_rgba(234,88,12,0.3)] disabled:opacity-50"
                  >
                    {isRedeeming ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Wallet className="w-3.5 h-3.5" />}
                    {t('ref.redeem.btn') || 'Redeem'}
                  </button>
                )}
              </div>
      ...
      {/* 3. Bottom Content: Activity Feed */}
      <div className="mt-8">
